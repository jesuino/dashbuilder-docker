...
public class SimplePrometheus implements DashboardGenerator {
    ...
    public Dashboard build() {
        ...
        var prometheus = DataSetDefFactory.newPrometheusDataSetDef()
                                          .uuid(UUID.randomUUID().toString())
                                          .serverUrl("http://localhost:9090")
                                          .query("prometheus_http_requests_total")
                                          .buildDef();

        var settings = DisplayerSettingsFactory.newBarChartSettings()
                                               .subType_Column()
        ...
        var page = PageFactory.page("p1", row("<h1>Http Requests</h1>"), row(settings));
